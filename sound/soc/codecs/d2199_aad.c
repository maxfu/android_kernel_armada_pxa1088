...
#if !defined(CONFIG_MACH_BAFFIN) && !defined(CONFIG_MACH_CS05)
...
static void d2199_aad_jack_monitor_timer_work(struct work_struct *work)
{
	...
	d2199_mask_irq(d2199_aad->d2199_codec->d2199_pmic, D2199_IRQ_EACCDET);
#endif	
...
out:
	...
	d2199_set_bits(d2199_aad->d2199_codec->d2199_pmic,D2199_EVENT_C_REG,
		D2199_E_ACC_DET_MASK);	
	d2199_unmask_irq(d2199_aad->d2199_codec->d2199_pmic, D2199_IRQ_EACCDET);
#endif
}
...
static void d2199_aad_button_monitor_timer_work(struct work_struct *work)
{
	...
	if (btn_status == 0 &&
		(jack_mode & D2199_ACCDET_JACK_MODE_JACK) == 0 ){ 
