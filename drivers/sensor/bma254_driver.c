...
#endif
static struct device *bma254_device;
		...
		struct bma254acc *acc)
{
	...
	printk("preraw x = %d, y = %d, z = %d, adjust = %d ",
...
static void bma254_set_enable(struct device *dev, int enable)
{
	...
	if (enable) {
		...
		#ifdef CONFIG_INPUT_BMA254_ACC_ALERT_INT
			if(!atomic_read(&bma254->reactive_enable)){
				bma254_set_mode(bma254->bma254_client, BMA254_MODE_SUSPEND);	
			}
		#else
			bma254_set_mode(bma254->bma254_client, BMA254_MODE_SUSPEND);
		#endif
		...
		struct device_attribute *attr, char *buf)
{
	...
	printk(KERN_INFO "bma254_calibration_show");
	...
	printk(KERN_INFO "bma254_calibration_show %d  %d %d %d\n",result,cal_data[0],cal_data[1],cal_data[2]);
		...
		const char *buf, size_t count)
{
	...
	printk(KERN_INFO "bma254_calibration_store");
	if(data){
		...
		if (bma254_set_offset_target_x(bma254->bma254_client, (unsigned
						char)0) < 0)
			return -EINVAL;
		...
		if (bma254_set_offset_target_y(bma254->bma254_client, (unsigned
						char)0) < 0)
			return -EINVAL;
...
#if defined (BMA254_MINUS_SIGNED_Z) || defined(CONFIG_MACH_DELOS_CTC)
		/* check the current z value and set offset_target_z based on it */
		if (bma254_set_offset_target_z(bma254->bma254_client, offset_target_z) < 0)
			return -EINVAL;
#else
		if (bma254_set_offset_target_z(bma254->bma254_client, (unsigned char)1) < 0)
			return -EINVAL;
#endif
	...
	/* calibration */
		bma254_get_offset_target_x(bma254->bma254_client, (unsigned char*)&cal_data[0]);
		bma254_get_offset_target_y(bma254->bma254_client, (unsigned char*)&cal_data[1]);
		bma254_get_offset_target_z(bma254->bma254_client, (unsigned char*)&cal_data[2]);
	...
	else
	{
		...
		set_fs(old_fs);
		    ...
		    bma254_set_offset_target_x(bma254->bma254_client, (unsigned char)cal_data[0]);
	        bma254_set_offset_target_y(bma254->bma254_client, (unsigned char)cal_data[1]);
        	bma254_set_offset_target_z(bma254->bma254_client, (unsigned char)cal_data[2]);		
	}
...
static void bma254_set_motion_interrupt(struct i2c_client *client, bool enable, bool factorytest)
{
	struct bma254_data *bma254 = i2c_get_clientdata(client);
	int comres;
        unsigned char data;
	if (enable) {
		...
		data = 0x04;
		comres = bma254_smbus_write_byte(client, BMA254_EN_INT1_PAD_SLOPE__REG,
                                                    &data);
		mdelay(3);
		...
		if (factorytest){
			data = 0x00;
			comres = bma254_smbus_write_byte(client, BMA254_SLOPE_THRES__REG,
								&data);
		}
		else{
		...
		mdelay(3);		
		data = 0x07;
		comres = bma254_smbus_write_byte(client, BMA254_INT_ENABLE1_REG,
		                                        &data);
	} 
	else {
		data = 0x00;
		...
		if(atomic_read(&bma254->factory_test)){
			...
			bma254_set_mode(bma254->bma254_client, BMA254_MODE_NORMAL);
		}else{
			bma254_set_mode(bma254->bma254_client, BMA254_MODE_SUSPEND);
		}
		mdelay(5);
						...
						*attr, char *buf)
{
	...
	pr_info("%s: state =%d onoff=%d\n", __func__,
		atomic_read(&bma254->reactive_state),atomic_read(&bma254->reactive_enable));
							...
							size_t count)
{
    ...
    #endif
	if (bma254->IRQ)
		bma254_set_motion_interrupt(bma254->bma254_client, onoff, factory_test);
	mdelay(1);
...
static DEVICE_ATTR(name, S_IRUGO|S_IWUSR|S_IWGRP, bma254_accel_name_show, NULL);
static DEVICE_ATTR(raw_data, S_IRUGO|S_IWUSR|S_IWGRP, bma254_value_show, NULL);
#ifdef CONFIG_INPUT_BMA254_ACC_ALERT_INT
static DEVICE_ATTR(reactive_alert, S_IRUGO|S_IWUSR|S_IWGRP,
		bma254_reactive_enable_show, bma254_reactive_enable_store);
#endif

static struct attribute *bma254_attributes_sensors[] = {
	&dev_attr_range.attr,
...
static void bma254_work_func_alert(struct work_struct *work)
{
	...
	if (result || bma254->factory_mode) {
		...
		wake_lock_timeout(&bma254->reactive_wake_lock,	msecs_to_jiffies(2000));
	}
	pr_info("%s: =============4\n", __func__);
...
//	struct bma254_platform_data *pdata = bma254->pdata;
	...
	if (irq > 0) {
		rc = request_threaded_irq(irq,	NULL, bma254_acc_irq_thread,
			IRQF_TRIGGER_RISING | IRQF_ONESHOT/*| IRQF_NO_SUSPEND*/,"accelerometer", bma254);
		pr_info("%s: irq = %d\n", __func__, irq);
		...
		const struct i2c_device_id *id)
{
	...
	err = sensors_register(&bma254_device, data, bma254_attributes_sensors, "accelerometer_sensor");
	if (err < 0) {
	...
	/*
	data->dev = sensors_register("accelerometer_sensor");
	if (IS_ERR(data->dev)) {
	...
	*/
/*
	err = device_create_file(&bma254_device, &dev_attr_name);
	if (err < 0) {
	...
	err = device_create_file(&bma254_device, &dev_attr_raw_data);
	if (err < 0) {
		pr_err("%s: Failed to create device file(%s)\n",
...
{
	printk(KERN_ERR "VVVVVVVVVVVVVVVVVVVVVVVVVVVVVV BMA254_init~ =====\n");
