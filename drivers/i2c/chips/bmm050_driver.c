...
static DEVICE_ATTR(name, S_IRUGO|S_IWUSR,
		bmm_read_name, NULL);
static DEVICE_ATTR(vendor, S_IRUGO|S_IWUSR,
...
static struct attribute *bmm_attributes_sensors[] = {
	&dev_attr_chip_id.attr,
	&dev_attr_op_mode.attr,
	&dev_attr_odr.attr,
	&dev_attr_rept_xy.attr,
...
static int bmm_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (NULL != client->dev.platform_data) {
		...
		if (NULL != client_data->bst_pd) {
			...
			//PINFO("platform data of bmm %s: place: %d, irq: %d",client_data->bst_pd->name,client_data->bst_pd->place,client_data->bst_pd->irq);
			PINFO("platform data of bmm %s: place: %d",client_data->bst_pd->name,client_data->bst_pd->place);
		}
	}
#endif
