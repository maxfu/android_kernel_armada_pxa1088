...
#elif defined(CONFIG_MACH_KYLE) || defined(CONFIG_MACH_ARUBA_TD) || defined(CONFIG_MACH_WARUBA) || defined(CONFIG_MACH_HARRISON) || defined(CONFIG_MACH_CS02)
...
#if defined(CONFIG_MACH_KYLE) || defined(CONFIG_MACH_ARUBA_TD) || defined(CONFIG_MACH_WARUBA) \
	|| defined(CONFIG_MACH_HARRISON) || defined(CONFIG_MACH_HELANDELOS) || defined(CONFIG_MACH_WILCOX) \
...
static void gp2a_prox_work_func(struct work_struct *work)
{
	...
	input_report_abs(gp2a->proximity_input_dev, ABS_DISTANCE, gp2a->val_state);
	input_sync(gp2a->proximity_input_dev);
	msleep(20);
...
static int gp2a_setup_irq(struct gp2a_data *gp2a)
{
	...
	gp2a_dbgmsg("start\n");
	printk("%s gp2a->pdata->p_out:%d", __func__,gp2a->pdata->p_out/*->i2c_client->irq*/);

	rc = gpio_request(gp2a->pdata->p_out, "gpio_proximity_out");
	if (rc < 0) {
		pr_err("%s: gpio %d request failed (%d)\n",
			__func__, gp2a->pdata->p_out, rc);
		return rc;
	}

	rc = gpio_direction_input(gp2a->pdata->p_out);
	if (rc < 0) {
	...
	value = 0x18;
	gp2a_i2c_write(gp2a, REGS_CON, &value);
	irq = gp2a->i2c_client->irq;//gpio_to_irq(gp2a->i2c_client->irq);
	rc = request_irq(irq,
			 gp2a_irq_handler,
