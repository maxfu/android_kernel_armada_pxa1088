...
static inline int rt8547_send_bit(struct rt8547_platform_data *pdata, unsigned char bit)
{
	if (bit > 0)
	{
		gpio_direction_output(pdata->flset_gpio, 0);
		udelay(SHORT_DELAY);
		gpio_direction_output(pdata->flset_gpio, 1);
		udelay(LONG_DELAY);
	}
	else
	{
...
static inline int rt8547_flash_en(int en)
{
	struct rt8547_platform_data *pdata = g_chip_ptr->dev->platform_data;
	if (en > 0)
		gpio_direction_output(pdata->flen_gpio, 1);
	else
		gpio_direction_output(pdata->flen_gpio, 0);
	RT_DBG("%s: en = %d\n", __func__, en);
...
static int __devinit rt8547_chip_io_init(struct rt8547_chip *chip)
{
	...
	if (!gpio_is_valid(pdata->flen_gpio) || !gpio_is_valid(pdata->flset_gpio))
	{
