...
static int sr200pc20m_set_preview_status(void)
{
...
static int sr200pc20m_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	switch (ctrl->id)
	{
		...
		case V4L2_CID_SET_PREVIEW_STATUS:
			retval = sr200pc20m_set_preview_status();
			break;
		...
		struct device_attribute *attr, char *buf)
{
	...
	char cam_fw[] = "SR200PC20M_SR200PC20M\n";

	return sprintf(buf, "%s", cam_fw);
}
...
static ssize_t rear_camera_flash_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	char cam_flash[] = "SR200PC20M_NONE\n";
...
__ATTR(rear_camtype, S_IRUGO | S_IWUGO , rear_camera_type_show, NULL);

static struct device_attribute dev_attr_camfw_rear =
__ATTR(rear_camfw, S_IRUGO | S_IWUGO, rear_camera_fw_show, NULL);

static struct device_attribute dev_attr_flash_rear =
__ATTR(rear_flash, S_IRUGO | S_IWUGO, rear_camera_flash_show, NULL);
			...
			const struct i2c_device_id *did)
{
	...
	if (device_create_file(cam_dev_rear, &dev_attr_flash_rear) < 0) {
		dev_err(&client->dev, "Failed to create device file: %s\n",
				dev_attr_flash_rear.attr.name);
		goto SKIP_SYSFS;
	}
