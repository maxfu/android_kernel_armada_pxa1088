...
{
	char cam_fw[] = "SR130PC10_SR130PC10\n";
...
__ATTR(front_camtype, S_IRUGO | S_IWUGO, front_camera_type_show, NULL);

static struct device_attribute dev_attr_camfw_front =
__ATTR(front_camfw, S_IRUGO | S_IWUGO, front_camera_fw_show, NULL);


static struct device_attribute dev_attr_flash_front =
__ATTR(front_flash, S_IRUGO | S_IWUGO, front_camera_flash_show, NULL);
			...
			const struct i2c_device_id *did)
{
	...
	if (device_create_file(cam_dev_front, &dev_attr_flash_front) < 0) {
		dev_err(&client->dev, "Failed to create device file: %s\n",
				dev_attr_flash_front.attr.name);
		goto SKIP_SYSFS;
	}
	#endif
