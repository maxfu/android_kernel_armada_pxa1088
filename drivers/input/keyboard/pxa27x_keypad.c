...
static int pxa27x_keypad_quick_scan_matrix(struct pxa27x_keypad *keypad, int *code, int *value)
{
	struct pxa27x_keypad_platform_data *pdata = keypad->pdata;
	int row, col, num_keys_pressed = 0;
	uint32_t new_state[MAX_MATRIX_KEY_COLS];
...
scan:
	for (col = 0; col < pdata->matrix_key_cols; col++) {
		...
		bits_changed = pdata->matrix_key_state[col] ^ new_state[col];
		if (bits_changed == 0)
			continue;
		...
		for (row = 0; row < pdata->matrix_key_rows; row++) {
			if ((bits_changed & (1 << row)) == 0)
				continue;
			*code = keypad->keycodes[MATRIX_SCAN_CODE(row, col, MATRIX_ROW_SHIFT)];
			*value = !!(new_state[col] & (1 << row));
			ret = 0;
		}
	}

	memcpy(pdata->matrix_key_state, new_state, sizeof(new_state));
...
static irqreturn_t pxa27x_keypad_irq_handler(int irq, void *dev_id)
{
...
#ifdef CONFIG_SEC_DEBUG
	{
		int code, value;
		struct pxa27x_keypad_platform_data *pdata = keypad->pdata;
		if (kpc & KPC_ME && !pxa27x_keypad_quick_scan_matrix(keypad, &code, &value)) {
			pdata->isr_hook(code, value);
		}
	}
#endif
