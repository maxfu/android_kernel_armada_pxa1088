...
#endif
#define CY_CORE_MODE_CHANGE_TIMEOUT		200
#define CY_CORE_MODE_CHANGE_TRIES		5
...
#ifdef CONFIG_MACH_BAFFIN	// For Baffin

#define RAWCOUNT_2BYTE (0) // YKJ 20130621 ;(1) GEN4x, TSG4.BASE.V2, (0) TSG4.PANSAM.x
#define DIFFCOUNT_2BYTE (0) // YKJ 20130621 ;(1) GEN4x, TSG4.BASE.V2, (0) TSG4.PANSAM.x
...
#if (BUTTON_NUM)
#define NODE_X_NUM (26 + 1)
#else
#define NODE_X_NUM (26)
#endif
...
unsigned int global_idac[NODE_NUM];
#else
...
static int cyttsp4_get_mode(struct cyttsp4_core_data *cd,
		enum cyttsp4_hst_mode_bits mode_bits, int *mode)
{
	switch (mode_bits & CY_HST_MODE) {
	case CY_HST_OPERATE:
		*mode = CY_MODE_OPERATIONAL;
		break;
	case CY_HST_SYSINFO:
		*mode = CY_MODE_SYSINFO;
		break;
	case CY_HST_CAT:
		*mode = CY_MODE_CAT;
		break;
	default:
		dev_vdbg(cd->dev, "%s: invalid hstmode: %02X(%d)\n",
			__func__, mode_bits, mode_bits);
		return -EINVAL;
	}
	return 0;
}
...
{
	int try = CY_CORE_MODE_CHANGE_TRIES;
	u8 hst_mode;
	int new_mode;
	int rc = 0;
	long t;

	dev_vdbg(cd->dev, "%s: wait mode=%d...\n", __func__, mode);
	...
	t = wait_event_timeout(cd->wait_q, cd->mode == mode,
		...
		/* Check new mode */
		if ((rc = cyttsp4_get_mode(cd, hst_mode, &new_mode)) == 0
				&& mode == new_mode) {
		mutex_lock(&cd->system_lock);
			cd->mode = new_mode;
		cd->int_status &= ~CY_INT_MODE_CHANGE;
	...
	dev_err(cd->dev, "%s: tmo waiting mode change, mode=%d cd->mode=%d\n",
		__func__, mode, cd->mode);
		...
		u8 new_dev_mode, int new_op_mode)
{
	...
	rc = cyttsp4_wait_mode_change(cd, new_op_mode);
...
static int cyttsp4_core_sleep_(struct cyttsp4_core_data *cd)
{
	...
	if (cd->pdata->power) {
		dev_err(cd->dev, "%s: Power down HW\n", __func__);
