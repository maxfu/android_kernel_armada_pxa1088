...
#endif
#if defined(TOUCH_BOOSTER)
struct pm_qos_request			tsp_cpufreq_qos_req_min;
#endif
...
static void cyttsp4_get_mt_touches(struct cyttsp4_mt_data *md, int num_cur_tch)
{
	...
	for (i = 0; i < num_cur_tch; i++) {
		...
		/* for ghost touch debugging */
#if 0
		if (tch.abs[CY_TCH_E] == CY_EV_LIFTOFF)		
			   printk("%s: tcn=%d, id=%d, x=%d, y=%d, RELEASE\n",__func__, num_cur_tch, tch.abs[CY_TCH_T], tch.abs[CY_TCH_X], tch.abs[CY_TCH_Y]);
		if (tch.abs[CY_TCH_E] == CY_EV_TOUCHDOWN)
			   printk("%s: tcn=%d, id=%d, x=%d, y=%d, PRESS\n",__func__, num_cur_tch, tch.abs[CY_TCH_T], tch.abs[CY_TCH_X], tch.abs[CY_TCH_Y]);
#endif
...
static int cyttsp4_xy_worker(struct cyttsp4_mt_data *md)
{
    ...
    static bool flag_booster = false;
	...
	if (num_cur_tch)
        {
#if defined(TOUCH_BOOSTER)
		      if (flag_booster == false)
		    {
		        flag_booster = true;
		       // printk("cyttsp4 %s: TSP_BOOSTER:: tcn=%d PRESS \n", __func__, num_cur_tch);
		#if defined(CONFIG_CPU_PXA988)
			pm_qos_update_request(&tsp_cpufreq_qos_req_min, 1066);
		#endif
		#if defined(CONFIG_CPU_PXA1088)
		        pm_qos_update_request(&tsp_cpufreq_qos_req_min, 1183);
		#endif
		    }
#endif
	...
	else
	{
		cyttsp4_lift_all(md);
#if defined(TOUCH_BOOSTER)
		 if(flag_booster == true)
		 {
		            flag_booster = false;
		            //printk(" cyttsp4 %s: TSP_BOOSTER:: tcn=%d  RELEASE\n",__func__, num_cur_tch);
		            pm_qos_update_request(&tsp_cpufreq_qos_req_min, PM_QOS_DEFAULT_VALUE);
		}
#endif
	}
	dev_vdbg(dev, "%s: done\n", __func__);
...
/*	pm_runtime_get(dev); PM runtime is not working*/
...
/*	pm_runtime_put(dev);   PM runtime is not working*/
}
...
static int cyttsp4_mt_probe(struct cyttsp4_device *ttsp)
{
	...
	pm_runtime_forbid(dev); /*PM runtime is not working*/
...
#if defined(TOUCH_BOOSTER)
	pm_qos_add_request(&tsp_cpufreq_qos_req_min, PM_QOS_CPUFREQ_MIN, PM_QOS_DEFAULT_VALUE);
#endif
