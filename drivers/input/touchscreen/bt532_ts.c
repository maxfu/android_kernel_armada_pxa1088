...
static void ts_tmr_work(struct work_struct *work)
{
	...
	disable_irq(info->irq);
	dev_err(&client->dev, "Timeout occured. TSP device reset & reinit\n");
	bt532_power_control(info, POWER_OFF);
	bt532_power_control(info, POWER_ON_SEQUENCE);

#if defined(TSP_VERBOSE_DEBUG)
	dev_info(&client->dev, "%s: Clear all reported points\n", __func__);
#endif
...
retry_init:
	...
	if (write_reg(client,
		BT532_BUTTON_SUPPORTED_NUM,
		(u16)cap->button_num) != I2C_SUCCESS)
		goto fail_init;

	if (write_reg(client,
		BT532_SUPPORTED_FINGER_NUM,
	...
	if (write_reg(client,
		BT532_INITIAL_TOUCH_MODE, TOUCH_POINT_MODE) != I2C_SUCCESS)
		goto fail_init;

	if (write_reg(client,
		BT532_TOUCH_MODE, info->touch_mode) != I2C_SUCCESS)
		goto fail_init;
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = read_data(misc_info->client, BT532_BUTTON_SENSITIVITY,
			(u8*)&threshold, 2);

	if (ret < 0) {
		dev_err(&client->dev, "%s: Failed to read key threshold\n",
					__func__);
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = read_data(client, BT532_BTN_WIDTH + 1, (u8*)&val, 2);
	if (ret < 0) {
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = read_data(client, BT532_BTN_WIDTH + 0, (u8*)&val, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s: Failed to read key sensitivity\n",
					__func__);
		info->work_state = NOTHING;
...
static int init_sec_factory(struct bt532_ts_info *info)
{
	...
	if (unlikely(!factory_info || !raw_data)) {
		dev_err(&info->client->dev, "Failed to allocate memory\n");
		ret = -ENOMEM;
		goto err_alloc;
	}
...
/* FW version read from tsp */
	...
	input_dev->name = BT532_TS_DEVICE;
...
/*	input_set_abs_params(info->input_dev, ABS_MT_ANGLE,
	...
	input_set_abs_params(info->input_dev, ABS_MT_TRACKING_ID,
		0, info->cap_info.multi_fingers, 0, 0);
