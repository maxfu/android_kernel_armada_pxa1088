...
woal_process_tcp_ack(moal_private * priv, mlan_buffer * pmbuf)
{
	...
	if (*((t_u8 *) tcph + 13) == 0x10) {
		...
		if (likely(ack_seq > tcp_session->ack_seq) && (skb->len == pmbuf->data_len)) {
			skb = (struct sk_buff *)tcp_session->ack_skb;
...
woal_dump_firmware_info(moal_handle * phandle)
{
	...
	pfileITCM = filp_open("/data/log/file_ITCM", O_CREAT | O_RDWR, 0644);
	if (IS_ERR(pfileITCM)) {
		PRINTM(MMSG,
		...
		PRINTM(MMSG,
		       ...
		       sec, usec);
		pfileDTCM =
			filp_open("/data/log/file_DTCM", O_CREAT | O_RDWR, 0644);
		pfileSQRAM =
			filp_open("/data/log/file_SQRAM", O_CREAT | O_RDWR, 0644);
	}
	if (IS_ERR(pfileITCM)) {
