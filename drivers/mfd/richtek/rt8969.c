...
#ifdef CONFIG_MFD_D2083 //support Dialog D2083 PMIC
#include <linux/d2083/d2083_battery.h>
#endif

#define DEVICE_NAME "rt8969"
#define RT8969_DRV_NAME "rt8969"

static struct rt8969_data* pDrvData = NULL; /*driver data*/
...
#ifdef CONFIG_MFD_D2083 //support Dialog D2083 PMIC
void d2083_enable_charge(enum power_supply_type type)
{
    struct i2c_client* pClient;
    int32_t regDev1;
    if (pDrvData) // driver has been initialized
    {
         pClient = pDrvData->client;
         regDev1 = I2CRByte(RT8969_REG_DEVICE_1);
         if (regDev1&0x40) // DCP
         {
             I2CWByte(RT8969_REG_CHG_CTRL1,CONFIG_RT8969_DCP_CHG_CTRL1);
             I2CWByte(RT8969_REG_CHG_CTRL2,CONFIG_RT8969_DCP_CHG_CTRL2);
             pDrvData->accessory_id = ID_CHARGER;
             INFO("[WS] ACCESSORY ID : CHARGER (DCP)\r\n");
             return;
         }
         else if (regDev1&0x20) // CDP
         {
             I2CWByte(RT8969_REG_CHG_CTRL1,CONFIG_RT8969_CDP_CHG_CTRL1);
             I2CWByte(RT8969_REG_CHG_CTRL2,CONFIG_RT8969_CDP_CHG_CTRL2);
             pDrvData->accessory_id = ID_CHARGER;
             INFO("[WS] ACCESSORY ID : CHARGER (CDP)\r\n");
             return;
          }
          else if (regDev1&0x10) //TA or Car Kit
          {
              I2CWByte(RT8969_REG_CHG_CTRL1,CONFIG_RT8969_TA_CHG_CTRL1);
              I2CWByte(RT8969_REG_CHG_CTRL2,CONFIG_RT8969_TA_CHG_CTRL2);
              pDrvData->accessory_id = ID_CHARGER;
			  INFO("[WS] ACCESSORY ID : CHARGER (TA/Car kit)\r\n");
              return;
          }
          else if (regDev1&0x04) // Standard USB Port
          {
              I2CWByte(RT8969_REG_CHG_CTRL1,CONFIG_RT8969_TA_CHG_CTRL1);
              I2CWByte(RT8969_REG_CHG_CTRL2,CONFIG_RT8969_TA_CHG_CTRL2);
              pDrvData->accessory_id = ID_USB;
			  INFO("[WS] ACCESSORY ID : ID_USB\r\n");
              return;
          }
          else
          {
              WARNING("Uknown charger : use default setting\r\n");
              I2CWByte(RT8969_REG_CHG_CTRL1,0xc8);
              I2CWByte(RT8969_REG_CHG_CTRL2,0x21);
              pDrvData->accessory_id = ID_CHARGER;
			  INFO("[WS] ACCESSORY ID : CHARGER (Unknow)\r\n");
              return;
          }
    }
}
EXPORT_SYMBOL(d2083_enable_charge);
void d2083_disable_charge(unsigned char end_of_charge)
{
    struct i2c_client* pClient;
    if (pDrvData) // driver has been initialized
    {
        pClient = pDrvData->client;
        if (end_of_charge == BAT_END_OF_CHARGE_NONE || end_of_charger == BAT_END_OF_CHARGE_BY_TIMER
            || end_of_charge == BAT_END_OF_CHARGE_BY_QUICKSTART || end_of_charge == BAT_END_OF_CHARGE_BY_FULL
            || end_of_charge == BAT_END_OF_CHARGE_BY_VF_OPEN)
        {
            I2CWByte(RT8969_REG_CHG_CTRL1,0x48);
            I2CWByte(RT8969_REG_CHG_CTRL2,0x21);
        }
    }
}
EXPORT_SYMBOL(d2083_disable_charge);
#endif
                               ...
                               const char *buf, size_t len)
{
    ...
    int32_t regCtrl1 = I2CRByte(RT8969_REG_CONTROL_1);
    pDrvData->operating_mode = value;
    if (value)
        regCtrl1 &= (~0x04); // Manual Contrl
    else
        regCtrl1 |= 0x04;// Auto Control
    I2CWByte(RT8969_REG_CONTROL_1,regCtrl1);
    return len;
}
...
static struct attribute* rt8969_attrs [] =
{
...
static struct kobj_attribute reg0x20_attribute = (struct kobj_attribute)__ATTR_RW(reg0x20);
static struct kobj_attribute reg0x21_attribute = (struct kobj_attribute)__ATTR_RW(reg0x21);
static struct kobj_attribute reg0x22_attribute = (struct kobj_attribute)__ATTR_RW(reg0x22);
static struct kobj_attribute reg0x23_attribute = (struct kobj_attribute)__ATTR_RO(reg0x23);
static struct kobj_attribute reg0x24_attribute = (struct kobj_attribute)__ATTR_RO(reg0x24);
static struct kobj_attribute enable_irq_attribute = (struct kobj_attribute)__ATTR_WO(enable_irq);
static struct attribute* rt8969_dbg_attrs [] =
{
    ...
    &reg0x20_attribute.attr,
    &reg0x21_attribute.attr,
    &reg0x22_attribute.attr,
    &reg0x23_attribute.attr,
    &reg0x24_attribute.attr,
    &accessory_attribute.attr,
    &chip_id_attribute.attr,
    &driver_version_attribute.attr,
    &factory_mode_attribute.attr,
...
static struct attribute_group rt8969_dbg_attrs_group =
{
    .name = "dbg",
    .attrs = rt8969_dbg_attrs,
};
...
static int enable_interrupt(int32_t enable)
{
    struct i2c_client* pClient = pDrvData->client;
    int32_t regCtrl1;
    regCtrl1 = I2CRByte(RT8969_REG_CONTROL_1);
    if (enable)
        regCtrl1 &= (~0x01);
    else
        regCtrl1 |= 0x01;
    return I2CWByte(RT8969_REG_CONTROL_1,regCtrl1);
...
inline void do_attach_work(int32_t regIntFlag,int32_t regDev1,int32_t regDev2)
{
    ...
    int32_t regCtrl1;
    struct i2c_client* pClient = pDrvData->client;
    if (regIntFlag&RT8969_INT_DCDTIMEOUT_MASK)
    {
        regADC = I2CRByte(RT8969_REG_ADC) & 0x1f;
        if (regADC == 0x1d || regADC == 0x1c
            || regADC == 0x19 || regADC == 0x18)
            {
                ...
                {
                    I2CWByte(RT8969_REG_MANUAL_SW1,0x6c);
                    I2CWByte(RT8969_REG_MANUAL_SW2,0x0c);
                }
                ...
                {
                    I2CWByte(RT8969_REG_MANUAL_SW1,0x6c);
                    I2CWByte(RT8969_REG_MANUAL_SW2,0x04);
                }
                pDrvData->accessory_id = ID_JIG;
                ...
                if (pDrvData->operating_mode)
                {
                    I2CWByte(RT8969_REG_MANUAL_SW1,0x24);
                    I2CWByte(RT8969_REG_MANUAL_SW2,0x0c);
                }
                pDrvData->accessory_id = ID_JIG;
        ...
        if (DCDT_retry>=MAX_DCDT_retry)
        {
            ...
            I2CWByte(RT8969_REG_INTERRUPT_MASK,0x08);
            I2CWByte(RT8969_REG_CHG_CTRL2,0x11);
            return;
        }
        I2CWByte(RT8969_REG_CONTROL_1,(uint8_t)(regCtrl1 & 0x9f));
        msleep(RT8969_WAIT_DELAY);
        I2CWByte(RT8969_REG_CONTROL_1,(uint8_t)(regCtrl1 | 0x60));
        DCDT_retry++;
        return;
    }
    if (regIntFlag&RT8969_INT_CHGDET_MASK)
    {
        if (regDev1&0x40)
        {
            I2CWByte(RT8969_REG_CHG_CTRL1,CONFIG_RT8969_DCP_CHG_CTRL1);
    ...
    if (regIntFlag&RT8969_INT_ADCCHG_MASK)
    {
        if (pDrvData->operating_mode)
        { // Manual Switch
            switch(regADC)
    ...
    {   //SDPort
        ...
        I2CWByte(RT8969_REG_CHG_CTRL1,CONFIG_RT8969_STDUSB_CHG_CTRL1);
        I2CWByte(RT8969_REG_CHG_CTRL2,CONFIG_RT8969_STDUSB_CHG_CTRL2);
        pDrvData->accessory_id = ID_USB;
...
inline void do_detach_work(int32_t regIntFlag,int32_t regCHGIntFlag)
{
    struct i2c_client* pClient = pDrvData->client;
    if (regIntFlag&0x40)
    {
        if (pDrvData->operating_mode)
        {
            I2CWByte(RT8969_REG_MANUAL_SW1,0);
            I2CWByte(RT8969_REG_MANUAL_SW2,0);
        }

    }
    else
    {
        msleep(3); //wait 2ms for EOC/attach confusion
        regCHGIntFlag = I2CRByte(RT8969_REG_CHG_INT_FLAG);
        if (regCHGIntFlag&0x80)
        {
            INFO("Unknown event!!\n");
        }
    }
    ...
    DCDT_retry = 0;
    I2CWByte(RT8969_REG_INTERRUPT_MASK,0x0);
    if (pDrvData->operating_mode)
    {
        I2CWByte(RT8969_REG_MANUAL_SW1,0);
        I2CWByte(RT8969_REG_MANUAL_SW2,0);
...
static irqreturn_t rt8969musc_irq_handler(int irq, void *data);


static void rt8969musc_work(struct work_struct *work)
{
    int32_t regIntFlag;
    int32_t regDev1,regDev2;
    int32_t regCHGIntFlag;
    struct i2c_client* pClient = pDrvData->client;
    struct rtmus_platform_data *pdata = &platform_data;
    regCHGIntFlag = I2CRByte(RT8969_REG_CHG_INT_FLAG);
    INFO("Interrupt CHG Flag = 0x%x\n",regCHGIntFlag);
    if (pDrvData->prev_chg_int_flag&0x80 && (regCHGIntFlag&0x80)==0)
    {
        msleep(3); //wait 2ms for EOC/attach confusion
        regIntFlag = I2CRByte(RT8969_REG_INT_FLAG);
        INFO("CHG_INT = 1 --> 0");
        INFO("Interrupt Flag = 0x%x\n",regIntFlag);
        if (regIntFlag&RT8969_INT_ATTACH_MASK)
        {
            regDev1 = I2CRByte(RT8969_REG_DEVICE_1);
            regDev2 = I2CRByte(RT8969_REG_DEVICE_2);
            if (regIntFlag&RT8969_INT_DETACH_MASK)
            {
                if (regDev1|regDev2)
                {
                    INFO("Attach\n");
                ...
                {
                    INFO("Detach\n");
                    do_detach_work(regIntFlag,regCHGIntFlag);
                }
        ...
        else if (regIntFlag&RT8969_INT_DETACH_MASK)
        { // Detach
            INFO("Detach\n");
            do_detach_work(regIntFlag,regCHGIntFlag);
        }
        else if (pDrvData->accessory_id)
        { // EOC
            I2CWByte(RT8969_REG_CHG_CTRL1,CONFIG_RT8969_CHGDONE_CHG_CTRL1);
            I2CWByte(RT8969_REG_CHG_CTRL2,CONFIG_RT8969_CHGDONE_CHG_CTRL2);
            INFO("Charging completed\n");
            if (likely(platform_data.charging_complete_callback))
                platform_data.charging_complete_callback();
        }
    }
    else
    {
        ...
        if (regIntFlag&RT8969_INT_ATTACH_MASK)
        {
            regDev1 = I2CRByte(RT8969_REG_DEVICE_1);
            regDev2 = I2CRByte(RT8969_REG_DEVICE_2);
            if (unlikely(regIntFlag&RT8969_INT_DETACH_MASK))
            {
    ...
    pDrvData->prev_chg_int_flag = regCHGIntFlag;
#if RT8969_IRQF_MODE == IRQF_TRIGGER_LOW // modify for samsung ivory
    enable_irq(pDrvData->irq);
#endif
...
static irqreturn_t rt8969musc_irq_handler(int irq, void *data)
{
	struct rt8969_data *pData = (struct rt8969_data*)data;
	wake_lock_timeout(&(pData->muic_wake_lock), 1*HZ);
#if RT8969_IRQF_MODE == IRQF_TRIGGER_LOW  // modify for samsung ivory
	disable_irq_nosync(irq);
#endif
	INFO("RT8969 interrupt triggered!\n");
...
static bool init_reg_setting(void)
{
    ...
    pDrvData->chip_id = I2CRByte(RT8969_REG_CHIP_ID);
    ...
    I2CWByte(RT8969_REG_RESET,0x01);
    msleep(RT8969_WAIT_DELAY);
    regCtrl1 = I2CRByte(RT8969_REG_CONTROL_1);
    I2CWByte(RT8969_REG_CHG_CTRL1,CHG_CTRL1_SETTING);
    I2CWByte(RT8969_REG_CHG_CTRL2,CHG_CTRL2_SETTING);
    I2CWByte(RT8969_REG_CHG_CTRL3,CHG_CTRL3_SETTING);
    INFO("reg_ctrl1 = 0x%x\n",regCtrl1);
    ...
    pDrvData->prev_int_flag = I2CRByte(RT8969_REG_INT_FLAG);
    pDrvData->prev_chg_int_flag = I2CRByte(RT8969_REG_CHG_INT_FLAG);
    INFO("prev_int_flag = 0x%x, prev_chg_int_flag = 0x%x\n",
         pDrvData->prev_int_flag,pDrvData->prev_chg_int_flag);
    enable_interrupt(1);
    msleep(RT8969_WAIT_DELAY);
...
static int rt8969musc_probe(struct i2c_client *client,
                        const struct i2c_device_id *id)
{
    int err;
    struct rt8969_data* drv_data;
    ...
    drv_data = kzalloc(sizeof(struct rt8969_data),GFP_KERNEL);
    ...
    rtmus_work_queue = create_workqueue("rt8969mus_wq");
    INIT_WORK(&drv_data->work, rt8969musc_work);
    ...
    err = gpio_request(CONFIG_RTMUSC_INT_GPIO_NUMBER,"RT8969_EINT");         // Request for gpio pin
    if (err<0)
    ...
    err = request_irq(client->irq, rt8969musc_irq_handler, RT8969_IRQF_MODE, DEVICE_NAME, drv_data);
    if (err < 0)
    {
...
static int rt8969musc_remove(struct i2c_client *client)
{
    if (pDrvData)
    {
        disable_irq(pDrvData->irq);
...
static void rt8969musc_shutdown(struct i2c_client *client)
{
    struct i2c_client *pClient = client;
    I2CWByte(RT8969_REG_RESET,0x01);
}

static int rt8969musc_resume(struct i2c_client *client)
{
	struct rtmus_platform_data *pdata = &platform_data;
/*
...
static int rt8969musc_suspend(struct i2c_client *client, pm_message_t state)
{
    struct rtmus_platform_data *pdata = &platform_data;
   /*
	if (device_may_wakeup(&client->dev) && client->irq)
...
static struct i2c_driver rt8969_i2c_driver =
{
    .driver = {
        .name = RT8969_DRV_NAME,
    },
    .probe = rt8969musc_probe,
    .remove = rt8969musc_remove,
#ifdef CONFIG_PM
    .shutdown = rt8969musc_shutdown,
	.resume = rt8969musc_resume,
	.suspend = rt8969musc_suspend,
#endif // CONFIG_PM
    .id_table = richtek_musc_id,
};

static int __init rt8969_init(void)
{

	int ret;
    ret = i2c_add_driver(&rt8969_i2c_driver);
    if (ret)
    {
    ...
    ret = rt_sysfs_create_files(&(rtmus_dev->dev.kobj),rt8969_attrs);
    if (ret)
        goto sysfs_create_fail;
#ifdef CONFIG_RT_SYSFS_DBG
    ret = sysfs_create_group(&(rtmus_dev->dev.kobj), &rt8969_dbg_attrs_group);
    if (ret)
        goto sysfs_create_fail;
#endif //CONFIG_RT_SYSFS_DBG

#ifdef CONFIG_MFD_D2083 //support Dialog D2083 PMIC
    INFO("Register D2038 charger enable/disable callback function.\n");
    if (d2083_register_enable_charge(&d2083_enable_charge)<0)
    {
        WWARNING("register disable charger callback function fail\r\n");
    }
    if (d2083_register_disable_charge(&d2083_disable_charge)<0)
    {
        WARNING("register disable charger callback function fail\r\n");
    }
#endif
	INFO("RT8969 Module initialized.\n");
    return 0;
sysfs_create_fail:
...
alloc_device_fail:
    i2c_del_driver(&rt8969_i2c_driver);
    ERR("RT8969 Module initialization failed.\n");
    return ret;
}
