...
void __init emeidkb_add_lcd_mipi(void)
{
	...
	case ID_S6D78A0X:
		...
		fb->modes->left_margin = 10;
		fb->modes->right_margin = 90;
		fb->modes->vsync_len = 8;
		fb->modes->upper_margin = 8;
		fb->modes->lower_margin = 16;
		fb->lcd_driver = &s6d78a0x_lcd_driver;
		fb->panel_rbswap = 1;
		break;
	case ID_HX8389B:
	default:
		printk("%s, hx8389b(id : 0x%x)\n", __func__, panel_id);
		fb->modes->hsync_len = 20;
		fb->modes->left_margin = 40;
		fb->modes->right_margin = 60;
		fb->modes->vsync_len = 1;
		fb->modes->upper_margin = 6;
		fb->modes->lower_margin = 18;
		fb->lcd_driver = &hx8389b_lcd_driver;
		fb->panel_rbswap = 0;
		break;
	}
