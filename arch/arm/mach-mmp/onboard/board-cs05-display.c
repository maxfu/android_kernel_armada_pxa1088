...
extern struct pxa168fb_mipi_lcd_driver hx8369_cs05_lcd_driver;
...
static struct pxa168fb_mach_info mipi_lcd_info = {
	...
	.phy_init = dsi_init,
	.phy_info = &dsiinfo,
	.lcd_driver = &hx8369_cs05_lcd_driver,
	.pxa168fb_lcd_power = lcd_power,
	.exter_brige_init = vx6b3e_brige_init,
	.exter_brige_pwr = vx6b3e_power,
	.width = 56,
	.height = 100,
};
		...
		unsigned int spi_gpio_reset, int on)
{
	...
	if (!lcd_vdd_3p0v) {
		lcd_vdd_3p0v = regulator_get(NULL, "v_lcd_3V");
		if (IS_ERR(lcd_vdd_3p0v)) {
