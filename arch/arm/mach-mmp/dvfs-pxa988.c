...
#ifdef D2199_DVFS	// DLG DVFS
#if defined(CONFIG_D2199_DVC)
	value = d2199_extern_dvc_write(level,regval);
#else
	ret = d2199_dvfs_set_voltage(level, regval);
#endif
#else
...
static int get_voltage_value(int level)
{
	...
	//dlg_trace("level[%d]\n", level);
#if defined(CONFIG_D2199_DVC)
	reg = d2199_extern_dvc_read(level);
#else
	reg = d2199_dvfs_get_voltage(level);
#endif
	if (reg < 0) {
...
#if defined(CONFIG_D2199_DVC)
	int volt = voltage_millivolts[buck_id - D2199_ID_BUCK1_AP_ACTIVE][*level];
#else
...
int d2199_dvc_set_voltage(int buck_id, int level)
{
	...
	if ((buck_id < D2199_ID_BUCK1_AP_ACTIVE) || (buck_id > D2199_ID_BUCK1_APSUB_SLEEP)) {
	...
	if (buck_id == D2199_ID_BUCK1_AP_ACTIVE) {
		reg_index = DVC_AP;
		high_bits = 1;
	} else if (buck_id == D2199_ID_BUCK1_AP_LPM) {
		reg_index = DVC_AP;
		high_bits = 0;
	} else if (buck_id == D2199_ID_BUCK1_APSUB_IDLE) {
		reg_index = DVC_APSUB;
		high_bits = 0;
	} else if (buck_id == D2199_ID_BUCK1_APSUB_SLEEP) {
		reg_index = DVC_APSUB;
		high_bits = 1;
	}
...
#ifdef D2199_DVFS	// DLG DVFS
#if defined(CONFIG_D2199_DVC)
	value = d2199_extern_dvc_write(0,volt_to_reg(vm_millivolts[0]));
#else
	value = d2199_dvfs_set_voltage(0, volt_to_reg(vm_millivolts[0]));
#endif
#else
...
//	volt1 = 1250;
	dlg_trace("volt1[%d]\n", volt1);
#if defined(CONFIG_D2199_DVC)
	volt1 = d2199_extern_dvc_read(volt1);
#else
	volt1 = d2199_dvfs_get_voltage(volt1);
#endif
