...
#if defined(CONFIG_MACH_HELANDELOS)
	pwd_sub_en_power = mfp_to_gpio(MFP_PIN_GPIO90);
#endif
...
#if defined(CONFIG_MACH_HELANDELOS)
	if(!pwd_sub_en_power) {
		printk(KERN_ERR "pwd_sub_en_power: %d\n", pwd_sub_en_power);
		return -1;
	}
#endif
...
#if defined(CONFIG_MACH_HELANDELOS)
	if (pwd_sub_en_power && gpio_request(pwd_sub_en_power, "pwd_sub_en_gpio90")) {
		printk(KERN_ERR "Request GPIO failed,"
				"gpio: %d\n", pwd_sub_en_power);
		pwd_sub_en_power = 0;
		return -1;;
